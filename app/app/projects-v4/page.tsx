...
export const metadata = { title: "Projects v004" };

import { getProjectMatrix } from "@/lib/db";
import { AntProjectTree } from "./ant-tree";
...
export default async function ProjectsV4Page() {
  const tasks = (await getProjectMatrix()) as Task[];
  const treeData = buildTree(tasks);

  return <AntProjectTree treeData={treeData} taskCount={tasks.length} />;
}
